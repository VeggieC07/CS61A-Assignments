...
def balanced(m):
    ...
    if is_planet(m):
        ...
        leftm, rightm = left(m), right(m)
        return balanced(end(leftm)) and balanced(end(rightm)) and length(leftm) * total_mass(end(leftm)) == length(rightm) * total_mass(end(rightm))
